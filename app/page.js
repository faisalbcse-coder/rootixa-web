...
const ToolCard = ({ tool }) => (
  <div className="group bg-white dark:bg-slate-800/90 p-6 rounded-3xl border-2 border-slate-100 dark:border-slate-700/50 shadow-sm hover:shadow-2xl hover:shadow-indigo-500/20 dark:hover:shadow-indigo-500/30 hover:border-indigo-500 dark:hover:border-indigo-400 hover:-translate-y-2 transition-all duration-300 flex flex-col h-full relative cursor-pointer overflow-hidden">
    
    <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-indigo-500/10 dark:from-indigo-500/20 to-transparent rounded-bl-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
    <div className="absolute -inset-px bg-gradient-to-r from-indigo-500 to-purple-500 rounded-3xl opacity-0 group-hover:opacity-10 transition-opacity duration-300 z-0 pointer-events-none"></div>
    ...
    <div className="relative z-10 w-14 h-14 bg-slate-50 dark:bg-slate-700/80 border border-slate-200 dark:border-slate-600/50 rounded-2xl flex items-center justify-center mb-5 group-hover:scale-110 group-hover:bg-indigo-50 dark:group-hover:bg-indigo-500/20 transition-all duration-300 shadow-sm group-hover:shadow-md">
      {tool.icon}
    </div>
...
export default function App() {
  ...
  const marqueeTools = [
    { name: 'Merge PDF', icon: <Archive className="w-4 h-4 text-violet-600 dark:text-violet-400" /> },
    { name: 'Background Remover', icon: <ImagePlus className="w-4 h-4 text-fuchsia-600 dark:text-fuchsia-400" /> },
    ...
    { name: 'Photo Resizer', icon: <ImagePlus className="w-4 h-4 text-indigo-600 dark:text-indigo-400" /> },
  ];
  ...
  const tools = [
    ...
    { id: 3, name: 'Image Resizer & Crop', desc: 'Resize, crop, and optimize images for any social media platform effortlessly.', icon: <ImagePlus className="w-6 h-6 text-indigo-600 dark:text-indigo-400" />, users: '3.2M', rating: 4.9, isNew: false, link: '#' },
    ...
    { id: 5, name: 'Image & PDF Converter', desc: 'Convert images to PDF or extract images from PDF documents seamlessly.', icon: <Archive className="w-6 h-6 text-violet-600 dark:text-violet-400" />, users: '4.1M', rating: 4.7, isNew: false, link: '#' },
    ...
    <div suppressHydrationWarning className={`min-h-screen font-sans flex flex-col transition-colors duration-500 ${darkMode ? 'dark bg-[#090E17] text-slate-200' : 'bg-white text-slate-800'}`}>
        ...
        <div className={`max-w-7xl mx-auto transition-all duration-500 flex justify-between items-center ${
          ...
          <div className="hidden lg:flex items-center space-x-1.5">
            ...
            >
              ...
              <div className={`absolute top-full left-1/2 -translate-x-1/2 pt-4 transition-all duration-300 z-50 ${desktopDropdownOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible translate-y-2'}`}>
                <div className="w-[360px] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-2xl shadow-indigo-500/10 dark:shadow-black/50 rounded-2xl overflow-hidden">
                  ...
                  <div className="p-2 flex flex-col max-h-[350px] overflow-y-auto no-scrollbar">
                    {tools.map(tool => (
                      <a key={tool.id} href={tool.link} className="flex items-center gap-3 p-3 hover:bg-indigo-50 dark:hover:bg-slate-800 rounded-xl transition-colors group">
                        <div className="w-10 h-10 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center group-hover:bg-white dark:group-hover:bg-slate-600 shadow-sm transition-colors flex-shrink-0">
                          {React.cloneElement(tool.icon, { className: "w-5 h-5 text-indigo-500" })}
                        </div>
                        <div>
                          <p className="text-sm font-bold text-slate-800 dark:text-slate-200 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 line-clamp-1 transition-colors">{tool.name}</p>
                  ...
                  <div className="p-3 bg-slate-50 dark:bg-slate-800/80 text-center border-t border-slate-100 dark:border-slate-800">
                    <a href="#" className="text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:underline flex items-center justify-center gap-1 transition-colors">Explore all tools <ArrowRight className="w-3 h-3" /></a>
                  </div>
                </div>
              </div>
            </div>
          ...
          <button className="lg:hidden p-2 text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 rounded-full transition-colors cursor-pointer" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
          ...
          <div className="lg:hidden absolute top-full left-4 right-4 mt-2 bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl border border-slate-100 dark:border-slate-800 shadow-2xl rounded-3xl p-5 flex flex-col space-y-3 z-50 animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            <div className="rounded-xl overflow-hidden bg-slate-50 dark:bg-slate-800/50">
              <button 
                ...
                className="w-full text-slate-800 dark:text-slate-200 font-bold hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-slate-800 px-4 py-3 flex items-center justify-between transition-colors cursor-pointer"
              >
              ...
              <div className={`transition-all duration-300 ease-in-out ${mobilePopularOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'} overflow-y-auto no-scrollbar`}>
                <div className="p-3 border-t border-slate-100 dark:border-slate-700/50 flex flex-col gap-2 bg-white dark:bg-slate-900">
                   {tools.map(tool => (
                     <a key={tool.id} href={tool.link} className="flex items-center gap-3 p-2 rounded-lg hover:bg-indigo-50 dark:hover:bg-slate-800 transition-colors cursor-pointer group">
                       <div className="w-8 h-8 rounded-md bg-indigo-50 dark:bg-slate-700 flex items-center justify-center flex-shrink-0 group-hover:bg-indigo-100 dark:group-hover:bg-slate-600 transition-colors">{React.cloneElement(tool.icon, { className: "w-4 h-4 text-indigo-500" })}</div>
                       <span className="text-sm font-bold text-slate-700 dark:text-slate-300 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors line-clamp-1">{tool.name}</span>
                     </a>
                   ))}
                </div>
              </div>
            ...
            <button className="bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-500/20 dark:hover:bg-indigo-500/30 text-indigo-600 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-500/30 px-5 py-4 rounded-2xl font-extrabold w-full flex justify-center items-center gap-2 transition-all shadow-sm cursor-pointer">
      ...
      <section className="relative pt-36 pb-16 md:pt-48 md:pb-24 overflow-hidden px-4">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <div className="w-full max-w-3xl mx-auto overflow-hidden fade-edges relative px-2 hover-pause-container">
            <div className="flex animate-marquee whitespace-nowrap py-4">
              {[...marqueeTools, ...marqueeTools, ...marqueeTools].map((tool, idx) => (
                <a key={`marquee-tool-${idx}`} href="#" className="mx-3 flex items-center gap-2 px-5 py-3 bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700/80 rounded-2xl cursor-pointer hover:border-violet-500 dark:hover:border-violet-400 hover:bg-violet-50 dark:hover:bg-slate-700 hover:shadow-lg hover:shadow-violet-500/20 hover:-translate-y-1 transition-all duration-300 shadow-sm group">
                  <div className="bg-slate-50 dark:bg-slate-700/80 p-1.5 rounded-xl group-hover:bg-white dark:group-hover:bg-slate-600 transition-colors">{tool.icon}</div>
        ...
        <div className="max-w-7xl mx-auto space-y-16 relative z-10">
          ...
          <div>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
              ...
              <a href="#" className="text-indigo-600 dark:text-indigo-400 font-bold hover:text-indigo-800 dark:hover:text-indigo-300 transition-colors flex items-center gap-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md hover:-translate-y-0.5 px-5 py-2.5 rounded-xl group cursor-pointer">
