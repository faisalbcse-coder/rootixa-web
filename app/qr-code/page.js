...
import PrintBusinessStudio from '@/components/qr/PrintBusinessStudio';
...
export default function QRCodeGenerator() {
  ...
  const [studioMode, setStudioMode] = useState('single'); // 'single' | 'print'
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <div className="inline-flex p-1 bg-slate-100 rounded-2xl border border-slate-200 shadow-2xs">
              ...
              >
                ...
                <span className="text-[9px] uppercase tracking-wider px-1.5 py-0.2 rounded-full bg-emerald-100 text-emerald-800 font-extrabold">
                  Phase 6
                </span>
              </button>
            </div>

            <span className="hidden sm:inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-xs">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pb-12">
        {studioMode === 'print' && (
          <PrintBusinessStudio
            ...
            onSwitchToSingle={() => setStudioMode('single')}
            withExportResolution={withExportResolution}
            blobToDataUrl={blobToDataUrl}
          />
        )}

        <div className={studioMode === 'single' ? 'grid grid-cols-1 lg:grid-cols-12 gap-8 items-start' : 'hidden'}>
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-white rounded-3xl p-6 sm:p-7 shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-slate-200/80 flex flex-col items-center">
                ...
                <div className="w-full space-y-3.5">
                  ...
                  {/* Quick Jump to Print & Business Studio */}
                  <div className="p-3 bg-gradient-to-r from-indigo-50/90 to-violet-50/90 border border-indigo-200/80 rounded-2xl flex items-center justify-between gap-3 shadow-2xs">
                    <div>
                      <p className="text-xs font-bold text-indigo-950 flex items-center gap-1.5">
                        <Printer className="w-3.5 h-3.5 text-indigo-600" /> Print Multiple Copies?
                      </p>
                      <p className="text-[10px] text-indigo-700">A4, A5, Letter sheets with cutting guides & cards</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setStudioMode('print')}
                      className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shrink-0 transition shadow-xs cursor-pointer hover:scale-105"
                    >
                      Print Studio →
                    </button>
                  </div>
